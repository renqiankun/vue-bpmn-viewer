...
const $ = (e, t) => {
  ...
  watch: {
    repainMap: {
      async handler({ xml: e, flow: t }) {
        await this.$nextTick(), e && this.preview();
      }
    }
  },
  mounted() {
  ...
  methods: {
    async preview() {
      ...
      const r = this.bpmnModeler;
      var i = await this.bpmnModeler.importXML(this.xml);
      if (i && i.error && i.error.length)
        return console.error(i);
      r.get("canvas").zoom("fit-viewport", "auto"), this.flow && this.flow.length > 0 && this.setMarker();
    },
    async setMarker() {
      await this.$nextTick(), this.canvas = this.bpmnModeler.get("canvas");
      var e = this.bpmnModeler.get("overlays"), t = this.bpmnModeler.get("elementRegistry"), r = {};
      this.flow.forEach((s) => {
        var l = s.activityId;
        r[l] = s;
      });
      for (var i in r) {
        var a = t.get(i);
        a && this.setMarkerHand(a, r[i], e);
      }
    },
    setMarkerHand(e, t, r) {
      var v, m, h, f;
      var i = e.id, a = (t == null ? void 0 : t.activityState) || "", s = `<div class='node-tool-tip-reset'> <div>${t.assignGroupName || ""}</div> $child$ </div>`, l = !1, n = t.activityState ? "<p></p>" : "";
      ...
      }), !l && t.assignDetailList.length > 0 && (n += `<div> ${(h = (m = t.assignDetailList) == null ? void 0 : m[0]) == null ? void 0 : h.createTime}</div>`), s = s.replace("$child$", n), r.add(i, {
        position: {
          top: e.height + 8 + (((f = e.label) == null ? void 0 : f.height) || 0),
          left: e.width / 2
        },
        html: s
      }), a == this.stateMap.reject)
        return this.canvas.addMarker(i, "node-reject");
      if (a == this.stateMap.agree)
        return this.canvas.addMarker(i, "node-pass");
      if (a == this.stateMap.review)
  ...
  class: "wrap",
  ref: "viewer",
  id: "bpmn-viewer-container"
};
function x(e, t, r, i, a, s) {
  return _(), g("div", k, null, 512);
}
const c = /* @__PURE__ */ $(b, [["render", x], ["__scopeId", "data-v-887e2a33"]]), u = (e) => {
